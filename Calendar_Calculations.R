...
special.days <- read_excel(
  ...
  sheet = "Special_Days",
  col_types = c(rep("numeric", 2), "text", "logical", rep("text", 2))
) %>%
...
calendar.generation <- function(year) {
  ...
  time.of.year$this.year$advent.start <-
    time.of.year$this.year$christmas$date -
    days(27 - (7 - time.of.year$this.year$christmas$day.of.week))
  ...
  time.of.year$next.year$advent.start <-
    time.of.year$next.year$christmas$date -
    days(27 - (7 - time.of.year$next.year$christmas$day.of.week))
...
time.of.year.attribution <- function(calendar) {
  ...
  moon.search$dates <-
    c(
      start = ymd(paste(start.year + 1, "03", "21", sep = "-")),
      end = ymd(paste(start.year + 1, "04", "25", sep = "-"))
    )
  moon.search$range <-
    seq(
      from = moon.search$dates["start"],
  ...
  moon.search$potential.easter <-
    moon.search$weekdays[
      moon.search$first.full +
        1:(length(moon.search$weekdays) - moon.search$first.full)
    ]
...
special.days.attribution <-
  function(calendar, special.days = special.days) {
    ...
    special.days <- special.days %>%
      mutate(
        year = case_when(month == 12 ~ start.year, TRUE ~ start.year + 1),
        .before = "month"
      ) %>%
      mutate(
        date = paste(
    ...
    search.date <-
      pull(calendar[
        max(which(calendar[["time.of.year"]] == "easter")),
        "date"
      ]) %m+%
      days(1)
    ...
    sundays$pos <-
      which(
        calendar[["time.of.year"]] == "ordinary" &
    ...
    {
      if (
        wday(ymd(paste(start.year, "12", "25", sep = "-")), week_start = 7) == 1
      ) {
        calendar[
          which(
            calendar[["date"]] == ymd(paste(start.year, "12", "30", sep = "-"))
          ),
          "type"
        ] <-
          "festivity"
        ...
        calendar[
          min(which(
            calendar[["time.of.year"]] == "christmas" &
              calendar[["day.of.week"]] == 1
          )),
          "type"
        ] <-
    ...
    calendar[max(which(calendar[["time.of.year"]] == "christmas")), "type"] <-
      "festivity"
    calendar[
      max(which(calendar[["time.of.year"]] == "christmas")),
      "name_german"
    ] <-
      "Taufe des Herrn"
    calendar[
    ...
    calendar[min(which(calendar[["time.of.year"]] == "lent")), "name_german"] <-
      "Aschermittwoch"
    calendar[
      min(which(calendar[["time.of.year"]] == "lent")),
      "name_english"
    ] <-
      "Ash Wednesday"
    ...
    calendar[max(which(calendar[["time.of.year"]] == "easter")) - 10, "type"] <-
      "high"
    calendar[
      max(which(calendar[["time.of.year"]] == "easter")) - 10,
      "name_german"
    ] <-
      "Christi Himmelfahrt"
    calendar[
    ...
    calendar[
      min(which(calendar[["time.of.year"]] == "easter")) - 7,
      "name_german"
    ] <-
      "Palmsonntag"
    calendar[
    ...
    calendar[
      min(which(calendar[["time.of.year"]] == "easter")) - 3,
      "name_german"
    ] <-
      "Gründonnerstag"
    calendar[
    ...
    calendar[
      min(which(calendar[["time.of.year"]] == "easter")) - 2,
      "name_german"
    ] <-
      "Karfreitag"
    calendar[
    ...
    calendar[
      min(which(calendar[["time.of.year"]] == "easter")) - 1,
      "name_german"
    ] <-
      "Karsamstag"
    calendar[
    ...
    calendar[min(which(calendar[["time.of.year"]] == "easter")), "type"] <-
      "high"
    calendar[
      min(which(calendar[["time.of.year"]] == "easter")),
      "name_german"
    ] <-
...
lithurgical.colours <- function(calendar) {
  ...
  pos <-
    append(
      pos,
      which(
        calendar[["name_german"]] %in%
          c("Kathedra Petri", "Bekehrung des Apostel Paulus")
      )
    )
  cols[pos] <- "white"
...
# Parameters:
# year: integer value for year in which calendar should start (defaults to the current year)
# special.days: a tibble with general dates of special days,
#               columns month, day, type, name_german, and name_english
#               by default stored in data/special_days.xlsx

catholic.calendar <- function(
